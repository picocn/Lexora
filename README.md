...
- **打印**：文件 → 打印…（Ctrl+Shift+P）。Markdown 可选择**打印预览版**（渲染后的排版：标题/表格/代码块/公式/图片）或**打印原始文本**（Markdown 源码）；其它文本文件直接打印原始文本。打印在独立窗口中完成，不包含应用界面与标签栏，并使用 **Windows 系统打印对话框**（可取消、可关闭）；若系统对话框不可用，可在打印窗口选择「在浏览器中打印」。
